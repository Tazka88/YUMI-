...
router.get('/admin/landing-pages', authenticate, async (req, res) => {
  try {
    const landingPages = await sql`
      SELECT lp.*, p.name as product_name, p.image as product_image 
...
router.post('/admin/landing-pages', authenticate, async (req, res) => {
  ...
  try {
    ...
    let count = 1;
    while (true) {
      const [existing] = await sql`SELECT id FROM landing_pages WHERE slug = ${finalSlug}`;
      if (!existing) break;
    ...
    const [landingPage] = await sql`
      ...
      VALUES (${product_id}, ${finalSlug}, '{}'::jsonb)
      RETURNING *
    `;
  ...
  try {
    const [landingPage] = await sql`
      SELECT lp.*, 
             ...
             p.image as product_image, p.faq_q1, p.faq_a1, p.faq_q2, p.faq_a2,
             p.features
      FROM landing_pages lp
      JOIN products p ON lp.product_id = p.id
      WHERE lp.slug = ${slug}
    ...
    const [reviewsData] = await sql`
      SELECT COUNT(id) as count, COALESCE(AVG(rating), 5) as average 
      FROM reviews 
      WHERE product_id = ${landingPage.product_id} AND status = 'approved'
    `;
    landingPage.reviews_count = parseInt(reviewsData.count) || 128;
    landingPage.reviews_average = parseFloat(reviewsData.average) || 4.8;
