...
const processImage = (table: string, id: number | string, field: string, image: string | null, slug?: string) => {
  ...
  if (image.startsWith('data:image/')) {
    ...
    // If slug is provided, we use the clean external URL /images/slug.webp
    // Otherwise fallback to the api internal one
    if (slug) {
...
const PRODUCT_COLS = `p.id, p.category_id, p.subcategory_id, p.sub_subcategory_id, p.brand_id, p.name, p.slug, p.description, p.price, p.promo_price, p.stock, CASE WHEN p.image LIKE 'data:image/%' THEN '/images/' || COALESCE(NULLIF(p.slug, ''), p.id::text) || '.webp' ELSE p.image END as image, p.video_url, p.is_popular, p.is_best_seller, p.is_new, p.is_recommended, p.is_fast_delivery, p.is_active, p.features, p.key_points, p.faq_q1, p.faq_a1, p.faq_q2, p.faq_a2, p.variations, p.created_at`;
...
const CATEGORIES_COLS = `id, name, slug, CASE WHEN image LIKE 'data:image/%' THEN '/images/' || COALESCE(NULLIF(slug, ''), id::text) || '.webp' ELSE image END as image, CASE WHEN slide_image LIKE 'data:image/%' THEN '/images/' || COALESCE(NULLIF(slug, ''), id::text) || '-slide.webp' ELSE slide_image END as slide_image, CASE WHEN mobile_slide_image LIKE 'data:image/%' THEN '/images/' || COALESCE(NULLIF(slug, ''), id::text) || '-mobile-slide.webp' ELSE mobile_slide_image END as mobile_slide_image`;
...
const BRANDS_COLS = `id, name, slug, description, CASE WHEN image LIKE 'data:image/%' THEN '/images/' || COALESCE(NULLIF(slug, ''), id::text) || '.webp' ELSE image END as image`;
const SUBCAT_COLS = `id, category_id, name, slug, CASE WHEN image LIKE 'data:image/%' THEN '/images/' || COALESCE(NULLIF(slug, ''), id::text) || '.webp' ELSE image END as image`;
const SUB_SUBCAT_COLS = `id, subcategory_id, name, slug, CASE WHEN image LIKE 'data:image/%' THEN '/images/' || COALESCE(NULLIF(slug, ''), id::text) || '.webp' ELSE image END as image`;
